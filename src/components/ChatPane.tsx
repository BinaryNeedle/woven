...
export default function ChatPane({ hostName, otherUsers }: Props) {
	const targetUserId = 2;
	const targetUser  = otherUsers.find((user: any) => user.user_id === targetUserId);
	...
	return (
		<section class="col-span-4 grid grid-rows-10 h-dvh">
			...
			<section class="px-5 mb-10 relative bg-[#0E8388] shadow-xl rounded-lg input-container-text flex">
				<input placeholder='Search or type a new chat' class="input-field focus:text-white" type="text" />
